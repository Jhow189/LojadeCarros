...
foreach ($_SESSION['carrinho'] as $cd => $qnt)  {
    $consulta = $cn->query("select vl_preco from tbl_carro where cd_carro='$cd'");
    $exibe = $consulta->fetch(PDO::FETCH_ASSOC);
    $preco = $exibe['vl_preco'];
    
	
	$inserir = $cn->query("insert into tbl_venda(no_Ticket,cd_cliente,cd_carro,qt_carro,vl_item,dt_venda)  
    values('$ticket','$cd_user','$cd','$qnt','$preco','$data')");
}
